...
- The `claude` binary was not present on PATH in this shell.
- The Claude Code npm entrypoint was available through `npm exec --yes @anthropic-ai/claude-code`.
...
- Claude summary generation logged a degraded `summary` hook error because the hook summary provider invokes `claude`, and only the npm entrypoint was available on PATH in this shell.
- The summary failure did not block base session capture or active linkage.
