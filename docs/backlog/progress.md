...
| Codex hook config location | Phase 4 | Closed 2026-05-06, updated 2026-05-14 | Codex docs support `hooks.json` and inline `[hooks]` but recommend one representation per config layer. Use user-level `~/.codex/hooks.json` for Stop hook commands and ensure `[features] hooks = true` in `~/.codex/config.toml`; Codex 0.130.0 warns that the older `codex_hooks` key is deprecated. |
