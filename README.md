...
By default, `foreman ui` binds to `127.0.0.1` on an OS-selected port and prints the local URL. The browser screens render overview and detail routes for tasks, chunks, dispatch runs, and sessions from specific API endpoints backed by existing `foreman ... --json` commands. Task and chunk status are shown separately from the dispatch gate, which only answers whether an open chunk is eligible for agent launch. Task detail includes compact chunk status/stage controls; chunk detail adds status/stage, notes, question add/answer, decision, and dispatch start controls. Dispatch detail adds confirmed lifecycle controls for finish/reconcile, queued cancel where available, workspace inspect and diff views, and separate confirmed merge/cleanup worktree actions. Every POST route maps to one allowlisted CLI command with `--json`, requires a same-origin `application/json` request, and does not query SQLite directly or mutate YAML/SQLite outside the CLI boundary. Hook install, Git push/pull, daemon behavior, and remote hosting remain outside the UI.
