...
By default, `foreman ui` binds to `127.0.0.1` on an OS-selected port and prints the local URL. The browser screens currently render overview and detail routes for tasks, chunks, dispatch runs, and sessions from specific API endpoints backed by existing `foreman ... --json` commands. The server also exposes a narrow local POST bridge for future controls: chunk status, chunk stage, chunk notes, question add/answer, and decision add. Every POST route maps to one allowlisted CLI command with `--json`; the server does not query SQLite directly or mutate YAML/SQLite outside the CLI boundary. Dispatch start, merge, cleanup, hook install, Git push/pull, daemon behavior, and remote hosting remain outside the UI.
